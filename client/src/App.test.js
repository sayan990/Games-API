...
describe("<Nav />", () => {
  ...
  xit('Debería renderizar dos <Link to="" />. El primero que vaya a "/", y el segundo a "/house/create"', () => {
    // Podes importar el componente Link de react-router-dom.
    expect(nav.find(Link).length).toBeGreaterThanOrEqual(2);
  });

  xit('Debería tener un Link con el texto "Home" que cambie la ruta hacia "/"', () => {
